...
# Render वरील DATABASE_URL वापरा, नसेल तर local sqlite वापरा (testing साठी)
...
DATABASE = os.path.join(os.path.dirname(__file__), 'kahoot_app.db')
DEPARTMENTS = ["Computer", "Mechanical", "Electrical", "Civil"]
OPENAI_BASE_URL = os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1").rstrip("/")
OPENAI_MODEL = os.environ.get("OPENAI_MODEL", "gpt-4o-mini")
# Keep one place for API key setup. Prefer env var, fallback lets you paste directly here.
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY", "").strip() or "PASTE_YOUR_OPENAI_API_KEY_HERE"
    ...
    with get_db_connection() as conn:
        ...
        if search_query:
            question_filter_clauses.append(
                "pa.user_id IN (SELECT user_id FROM Users WHERE role='Student' AND LOWER(username) LIKE ?)"
            )
            question_params.append(f"%{search_query.lower()}%")
        if selected_date:
            question_filter_clauses.append("date(pa.submitted_at)=?")
...
def teacher_practice_quiz_results_export(quiz_id):
    ...
    with get_db_connection() as conn:
        ...
        raw_rows = conn.execute(
            ...
            JOIN PracticeQuestions q ON q.question_id = pa.question_id
            WHERE pa.quiz_id=?
        ...
        latest_answers = conn.execute(
            ...
            JOIN (
                SELECT user_id, question_id, MAX(answer_id) AS latest_answer_id
                FROM PracticeAnswers
                WHERE quiz_id=?
...
def ai_generate_questions():
    ...
    if not api_key or api_key == "PASTE_YOUR_OPENAI_API_KEY_HERE":
        return jsonify({
            "ok": False,
            "error": "AI API key missing. Set OPENAI_API_KEY or update OPENAI_API_KEY in app.py."
        }), 400

    prompt = f"""
Generate exactly {num_questions} multiple-choice quiz questions on topic: "{topic}".
...
def update_quiz(quiz_id):
    ...
    try:
        ...
        for idx, (qid, qtext) in enumerate(zip(question_ids, question_texts)):
            ...
            if media_url:
                cur.execute(
                    "UPDATE Questions SET question_text=?, media_url=? WHERE question_id=?",
                    (qtext, media_url, qid)
                )
            else:
                cur.execute(
        ...
        if new_questions:
            ...
            for i, qtext in enumerate(new_q_texts):
                ...
                cur.execute(
                    "INSERT INTO Questions (quiz_id, question_text, media_url) VALUES (?, ?, ?)",
                    (quiz_id, qtext, media_url)
                )
